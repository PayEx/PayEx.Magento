...
class AAIT_Payex2_Model_Feed extends Mage_AdminNotification_Model_Feed
{
    const URL_NEWS = 'http://payex.aait.se/application/meta/check?key=PN1l332O6N2uttu';
    ...
    {
        return Mage::getModel('payex2/feed')->checkUpdate();
    }
    ...
    public function getFeedUrl()
    {
        ...
        $params = array_merge($params, $this->getInstalledPayExModules());
    ...
    /**
     * Get Installed PayEx Modules
     * @return array
     */
    public function getInstalledPayExModules()
    {
        $modules_list = array();
        ...
        foreach ($modules as $module => $config) {
            if ($config->is('active') && mb_strpos($module, 'AAIT_') !== false) {
                $modules_list[$module] = (string) $config->version;
            }
        }
