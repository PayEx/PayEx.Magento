...
class AAIT_PartPayment_PaymentController extends Mage_Core_Controller_Front_Action
{
    public function _construct()
    {
        Mage::getSingleton('partpayment/payment');
    }

    public function redirectAction()
    {
        Mage::helper('partpayment/tools')->addToDebug('Controller: redirect');
        ...
        $params = array(
            ...
            'clientLanguage' => Mage::getSingleton('partpayment/payment')->getConfigData('clientlanguage')
        );
        $result = Mage::helper('partpayment/api')->getPx()->Initialize8($params);
        Mage::helper('partpayment/tools')->addToDebug('PxOrder.Initialize8:' . $result['description']);

        // Check Errors
        if ($result['code'] !== 'OK' || $result['description'] !== 'OK' || $result['errorCode'] !== 'OK') {
            $message = Mage::helper('partpayment/tools')->getVerboseErrorMessage($result);
        ...
        $result = Mage::helper('partpayment/api')->getPx()->PurchasePartPaymentSale($params);
        Mage::helper('partpayment/tools')->addToDebug('PxOrder.PurchasePartPaymentSale:' . $result['description']);
        ...
        if (empty($result['transactionStatus']) || !is_numeric($result['transactionStatus'])) {
            $message = Mage::helper('partpayment')->__('Error: No transactionsStatus in response.');
            Mage::helper('partpayment/tools')->addToDebug('Error: No transactionsStatus in response.', $order->getIncrementId());
        ...
        if (in_array((int)$result['transactionStatus'], array(0, 3, 6)) && $order->getState() === Mage_Sales_Model_Order::STATE_CANCELED) {
            ...
            Mage::helper('partpayment/tools')->addToDebug('Order has been uncanceled.', $order_id);
        }

        // Process Transaction
        Mage::helper('partpayment/tools')->addToDebug('Process Payment Transaction...', $order_id);
        $transaction = Mage::helper('partpayment/order')->processPaymentTransaction($order, $result);
        ...
        switch ($transaction_status) {
            ...
            case 6:
                ...
                if (in_array($transaction_status, array(0, 6))) {
                    $invoice = Mage::helper('partpayment/order')->makeInvoice($order, false);
            ...
            default:
                ...
                if ($transaction->getIsCancel()) {
                    Mage::helper('partpayment/tools')->addToDebug('Cancel: ' . $transaction->getMessage(), $order->getIncrementId());
