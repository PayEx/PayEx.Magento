...
/**
 * PayEx Helper: Pending Cleaner
 * Created by AAIT Team.
 */
class AAIT_PartPayment_Helper_Cleaner extends Mage_Core_Helper_Abstract
{
            ...
            foreach ($results as $result) {
                if (isset($result['increment_id'])) {
                    ...
                    if (!$order->isCanceled() && !$order->hasInvoices()) {
                        ...
                        $order->cancel()->save();
                        // Add to Log
                        Mage::helper('partpayment/tools')->addToDebug('Pending Clean(' . $state . '): OrderID ' . $order_id . ' is canceled.');
                    }
                }
            }
        }
