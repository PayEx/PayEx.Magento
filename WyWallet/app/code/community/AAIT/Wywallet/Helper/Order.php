...
class AAIT_Wywallet_Helper_Order extends Mage_Core_Helper_Abstract
{
    ...
    public function processPaymentTransaction(Mage_Sales_Model_Order $order, array $fields)
    {
        ...
        if (count($collection) > 0) {
            Mage::helper('wywallet/tools')->addToDebug(sprintf('Transaction %s already processed.', $fields['transactionNumber']), $order->getIncrementId());
        ...
        switch ($transaction_status) {
            case 1:
                ...
                if ($fields['pending'] === 'true') {
                    $message = Mage::helper('wywallet')->__('Transaction Status: %s.', $transaction_status);
            ...
            case 6:
                ...
                break;
            case 2:
                $message = Mage::helper('wywallet')->__('Detected an abnormal payment process (Transaction Status: %s).', $transaction_status);
                $transaction = $payment->addTransaction(Mage_Sales_Model_Order_Payment_Transaction::TYPE_PAYMENT, null, true, $message);
                $transaction->setAdditionalInformation(Mage_Sales_Model_Order_Payment_Transaction::RAW_DETAILS, $fields);
                $transaction->setMessage($message);
                ...
                break;
            case 4;
                $message = Mage::helper('wywallet')->__('Order automatically canceled. Transaction is canceled.');
                ...
                break;
            case 5;
                $message = Mage::helper('wywallet')->__('Order automatically canceled. Transaction is failed.');
                $message .= ' ' . Mage::helper('wywallet/tools')->getVerboseErrorMessage($fields);
                $transaction = $payment->addTransaction(Mage_Sales_Model_Order_Payment_Transaction::TYPE_PAYMENT, null, true);
                $transaction->setAdditionalInformation(Mage_Sales_Model_Order_Payment_Transaction::RAW_DETAILS, $fields);
                $transaction->setMessage($message);
                $transaction->setIsCancel(true);
                ...
                break;
            default:
                $message = Mage::helper('wywallet')->__('Invalid transaction status.');
        ...
        try {
            $order->save();
            Mage::helper('wywallet/tools')->addToDebug($message, $order->getIncrementId());
        } catch (Exception $e) {
            Mage::helper('wywallet/tools')->addToDebug('Error: ' . $e->getMessage(), $order->getIncrementId());
        }
    ...
    public function addOrderLine($orderRef, $order)
    {
        ...
        // add Discount
        $discount = $order->getShippingDiscountAmount() + $order->getDiscountAmount();
        if (abs($discount) > 0) {
            $params = array(
                'accountNumber' => '',
                'orderRef' => $orderRef,
        ...
        if ((float)$order->getBaseRewardCurrencyAmount() > 0) {
            $params = array(
                ...
                'orderRef' => $orderRef,
                'itemNumber' => $i,
                'itemDescription1' => Mage::helper('wywallet')->__('Reward points'),
                'itemDescription2' => '',
                'itemDescription3' => '',
                'itemDescription4' => '',
                'itemDescription5' => '',
