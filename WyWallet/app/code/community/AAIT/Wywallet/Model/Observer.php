...
class AAIT_Wywallet_Model_Observer extends Mage_Core_Model_Abstract
{
    /**
    ...
    public function cleanPendingOrders()
    {
        ...
        $clean_time = -1 * (int)Mage::getSingleton('wywallet/payment')->getConfigData('cleantime');
        if ($clean_time !== 0) {
            // Force Cancel Pending orders
            $clean_time = date('Y-m-d H:i:s', strtotime($clean_time . ' minutes'));
            Mage::helper('wywallet/cleaner')->forceCancel('pending_payment', $clean_time);
            //Mage::helper('wywallet/cleaner')->forceCancel('pending', $clean_time);
        }
