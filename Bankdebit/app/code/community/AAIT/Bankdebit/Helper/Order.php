...
class AAIT_Bankdebit_Helper_Order extends Mage_Core_Helper_Abstract
{
    ...
    public function processPaymentTransaction(Mage_Sales_Model_Order $order, array $fields)
    {
        ...
        if (count($collection) > 0) {
            Mage::helper('bankdebit/tools')->addToDebug(sprintf('Transaction %s already processed.', $fields['transactionNumber']), $order->getIncrementId());
        ...
        switch ($transaction_status) {
            case 1:
                ...
                if ($fields['pending'] === 'true') {
                    $message = Mage::helper('bankdebit')->__('Transaction Status: %s.', $transaction_status);
            ...
            case 6:
                ...
                break;
            case 2:
                $message = Mage::helper('bankdebit')->__('Detected an abnormal payment process (Transaction Status: %s).', $transaction_status);
                $transaction = $payment->addTransaction(Mage_Sales_Model_Order_Payment_Transaction::TYPE_PAYMENT, null, true, $message);
                $transaction->setAdditionalInformation(Mage_Sales_Model_Order_Payment_Transaction::RAW_DETAILS, $fields);
                $transaction->setMessage($message);
                ...
                break;
            case 4;
                $message = Mage::helper('bankdebit')->__('Order automatically canceled. Transaction is canceled.');
                ...
                break;
            case 5;
                $message = Mage::helper('bankdebit')->__('Order automatically canceled. Transaction is failed.');
                $message .= ' ' . Mage::helper('bankdebit/tools')->getVerboseErrorMessage($fields);
                $transaction = $payment->addTransaction(Mage_Sales_Model_Order_Payment_Transaction::TYPE_PAYMENT, null, true);
                $transaction->setAdditionalInformation(Mage_Sales_Model_Order_Payment_Transaction::RAW_DETAILS, $fields);
                $transaction->setMessage($message);
                $transaction->setIsCancel(true);
                ...
                break;
            default:
                $message = Mage::helper('bankdebit')->__('Invalid transaction status.');
        ...
        try {
            $order->save();
            Mage::helper('bankdebit/tools')->addToDebug($message, $order->getIncrementId());
        } catch (Exception $e) {
            Mage::helper('bankdebit/tools')->addToDebug('Error: ' . $e->getMessage(), $order->getIncrementId());
        }
    ...
    public function addOrderLine($orderRef, $order)
    {
        ...
        if ((float)$order->getBaseRewardCurrencyAmount() > 0) {
            $params = array(
                ...
                'orderRef' => $orderRef,
                'itemNumber' => $i,
                'itemDescription1' => Mage::helper('bankdebit')->__('Reward points'),
                'itemDescription2' => '',
                'itemDescription3' => '',
                'itemDescription4' => '',
                'itemDescription5' => '',
