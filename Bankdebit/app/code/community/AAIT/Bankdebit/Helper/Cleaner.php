...
/**
 * PayEx Bank Debit Helper: Pending Cleaner
 * Created by AAIT Team.
 */
class AAIT_Bankdebit_Helper_Cleaner extends Mage_Core_Helper_Abstract
{
            ...
            foreach ($results as $result) {
                if (isset($result['increment_id'])) {
                    ...
                    if (!$order->isCanceled() && !$order->hasInvoices()) {
                        ...
                        $order->cancel()->save();
                        // Add to Log
                        Mage::helper('bankdebit/tools')->addToDebug('Pending Clean(' . $state . '): OrderID ' . $order_id . ' is canceled.');
                    }
                }
            }
        }
