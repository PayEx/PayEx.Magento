...
class PayEx_MasterPass_Helper_Cleaner extends Mage_Core_Helper_Abstract
{
            ...
            foreach ($results as $result) {
                if (isset($result['increment_id'])) {
                    ...
                    if (!$order->isCanceled() && !$order->hasInvoices()) {
                        ...
                        $order->cancel()->save();
                        // Add to Log
                        Mage::helper('payex_mp/tools')->addToDebug('Pending Clean(' . $state . '): OrderID ' . $order_id . ' is canceled.');
                    }
                }
            }
        }
